...
namespace BanHang.Data
{
    public class dtPhieuChuyenKho
    {
        ...
        public DataTable DanhSachPhieuChuyenKho_Tong(string s)
        {
            using (SqlConnection con = new SqlConnection(StaticContext.ConnectionString))
            {
                con.Open();
                string cmdText = " SELECT " + s + "* FROM [GPM_PhieuChuyenKho] WHERE IDKhoXuat is not null AND IDKhoNhap is not null AND DaXoa = 0 AND IDTrangThai = 1 AND SoMatHang is not null ORDER BY [ID] DESC";
                using (SqlCommand command = new SqlCommand(cmdText, con))
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    DataTable tb = new DataTable();
        ...
        public DataTable DanhSachPhieuChuyenKho_Client(string IDKho, string s)
        {
            using (SqlConnection con = new SqlConnection(StaticContext.ConnectionString))
            {
                con.Open();
                string cmdText = " SELECT " + s + "* FROM [GPM_PhieuChuyenKho] WHERE IDKhoXuat is not null AND IDKhoNhap is not null AND SoMatHang is not null AND DaXoa = 0 AND IDTrangThai = 1 AND (IDKhoXuat = '" + IDKho + "' OR IDKhoNhap = '" + IDKho + "') ORDER BY [ID] DESC";
                using (SqlCommand command = new SqlCommand(cmdText, con))
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    DataTable tb = new DataTable();
