...
namespace BanHang.Data
{
    public class dtKhoHang
    {
        public DataTable LayDanhSachHangTrongKho(string IDKho, string HienThi, string IDNhanVien)
        {
            ...
            {
                con.Open();
                string cmdText = "SELECT [GPM_HangHoaTonKho].*,[GPM_HangHoa].MaHang,[GPM_HangHoa].IDDonViTinh, GPM_HangHoa_Barcode.Barcode FROM GPM_HangHoa_Barcode, [GPM_HangHoaTonKho],[GPM_HangHoa] WHERE GPM_HangHoa_Barcode.IDHangHoa = GPM_HangHoa.ID AND [GPM_HangHoaTonKho].DaXoa = 0 AND [GPM_HangHoa].ID =  [GPM_HangHoaTonKho].IDHangHoa  AND (" + dtSetting.LayQuyenTruyCapKho_CoTenBang(IDNhanVien, "[GPM_HangHoaTonKho].") + " [GPM_HangHoaTonKho].IDKho = " + IDKho + ")";
