...
namespace BanHang.Data
{
    public class dtQuanTriNguoiDung
    {
        ...
        public void ThemNguoiDung(string TenNguoiDung, string TenDangNhap, int IDNhomNguoiDung, string SDT, string MatKhau, string IDKho)
        {
            using (SqlConnection myConnection = new SqlConnection(StaticContext.ConnectionString))
            {
                ...
                {
                    myConnection.Open();
                    string cmdText = "INSERT INTO [GPM_NGUOIDUNG] ([TenNguoiDung],[TenDangNhap], [IDNhomNguoiDung], [SDT], [MatKhau], [NgayCapNhat],[IDKho]) VALUES (@TenNguoiDung,@TenDangNhap, @IDNhomNguoiDung, @SDT, @MatKhau, getdate(),@IDKho)";
        ...
        public void SuaNguoiDung(int ID, string TenNguoiDung, string TenDangNhap,int IDNhomNguoiDung, string SDT, string MatKhau, string IDKho)
        {
            using (SqlConnection myConnection = new SqlConnection(StaticContext.ConnectionString))
            {
                try
                {
                    myConnection.Open();
                    string strSQL = "UPDATE [GPM_NGUOIDUNG] SET [IDNhomNguoiDung] = @IDNhomNguoiDung,[TenDangNhap] = @TenDangNhap,[TenNguoiDung] = @TenNguoiDung,[SDT] = @SDT, [NgayCapNhat] = getdate(), [MatKhau] = @MatKhau,[IDKho] = @IDKho WHERE [ID] = @ID";
                    using (SqlCommand myCommand = new SqlCommand(strSQL, myConnection))
                    {
                        myCommand.Parameters.AddWithValue("@ID", ID);
                        myCommand.Parameters.AddWithValue("@IDNhomNguoiDung", IDNhomNguoiDung);
