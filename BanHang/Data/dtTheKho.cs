...
namespace BanHang.Data
{
    public class dtTheKho
    {
        ...
        public static object ThemTheKho(string MaDonHang, string DienGiai, string NhapTrongKy, string XuatTrongKy, string TonCuoiKy, string IDNhanVien, string IDKho, string IDHangHoa, string LoaiPhieu, string XuatKhac, string XuatTra, string KiemKho, string DieuChinhStock)
        {
            using (SqlConnection myConnection = new SqlConnection(StaticContext.ConnectionString))
            {
                try
                {
                    object ID = null;
                    myConnection.Open();
                    string cmdText = "INSERT INTO [GPM_TheKho] ([MaDonHang], [NgayLap], [DienGiai], [NhapTrongKy],[XuatTrongKy],[TonCuoiKy], [IDNhanVien],[IDKho],[IDHangHoa],[LoaiPhieu],[XuatKhac],[XuatTra],[KiemKho],[DieuChinhStock])  OUTPUT INSERTED.ID  VALUES (@MaDonHang,getdate(),@DienGiai, @NhapTrongKy,@XuatTrongKy,@TonCuoiKy,@IDNhanVien,@IDKho,@IDHangHoa,@LoaiPhieu,@XuatKhac,@XuatTra,@KiemKho,@DieuChinhStock)";
                    using (SqlCommand myCommand = new SqlCommand(cmdText, myConnection))
                    {
                        myCommand.Parameters.AddWithValue("@DieuChinhStock", DieuChinhStock);
